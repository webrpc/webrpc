...
func WebRPCSchemaHash() string {
	return "e0b09ba1eb67fbeb7ea7315425944eaddf7b187f"
}
...
type ExampleClient interface {
	...
	UploadAvatar(ctx context.Context, userId uint64, avatar *WebrpcFile) (uint64, string, error)
	// DownloadAvatar returns the user's stored avatar image (file download).
	DownloadAvatar(ctx context.Context, userId uint64) (*WebrpcFile, error)
}
...
func (c *exampleClient) Ping(ctx context.Context, counter *BigInt) error {
	in := struct {
...
func (c *exampleClient) GetUser(ctx context.Context, userId uint64) (uint32, *User, error) {
	...
	err := doHTTPRequest(ctx, c.client, c.urls[1], in, &out)
...
func (c *exampleClient) GetArticle(ctx context.Context, getArticleRequest GetArticleRequest) (*GetArticleResponse, error) {
	...
	err := doHTTPRequest(ctx, c.client, c.urls[2], getArticleRequest, &out)
	return out, err
}

func (c *exampleClient) UploadAvatar(ctx context.Context, userId uint64, avatar *WebrpcFile) (uint64, string, error) {
	in := struct {
		Arg0 uint64      `json:"userId"`
		Arg1 *WebrpcFile `json:"avatar"`
	}{userId, avatar}

	out := struct {
		Ret0 uint64 `json:"size"`
		Ret1 string `json:"name"`
	}{}

	parts := []filePart{}
	parts = append(parts, filePart{name: "avatar", files: []*WebrpcFile{avatar}})
	err := doHTTPUploadRequest(ctx, c.client, c.urls[3], in, parts, &out)

	return out.Ret0, out.Ret1, err
}

func (c *exampleClient) DownloadAvatar(ctx context.Context, userId uint64) (*WebrpcFile, error) {
	in := struct {
		Arg0 uint64 `json:"userId"`
	}{userId}

	resp, err := doHTTPRequestRaw(ctx, c.client, c.urls[4], in, nil)
	if err != nil {
		if resp != nil {
			resp.Body.Close()
		}
...
func (s *exampleService) serveUploadAvatarMultipart(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	var formErr error
	reqPayload.Arg1, formErr = upload.file("avatar")
	if formErr != nil {
...
const WebrpcHeaderValue = "webrpc;golang;node-ts@v1.0.0"
