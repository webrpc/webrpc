...
export class Example implements ExampleClient {
  ...
  ping = (req: PingRequest, headers?: object, signal?: AbortSignal): Promise<PingResponse> => {
    return this.fetch(
      this.url('Ping'),
      createHttpRequest(JsonEncode('PingRequest', req), headers, signal)).then((res) => {
      return buildResponse(res).then(_data => {
        return JsonDecode<PingResponse>('PingResponse', _data)
      })
    }, (error) => {
  ...
  getUser = (req: GetUserRequest, headers?: object, signal?: AbortSignal): Promise<GetUserResponse> => {
    return this.fetch(
      this.url('GetUser'),
      createHttpRequest(JsonEncode('GetUserRequest', req), headers, signal)).then((res) => {
      return buildResponse(res).then(_data => {
        return JsonDecode<GetUserResponse>('GetUserResponse', _data)
      })
    }, (error) => {
      throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
    })
  ...
  getArticle = (req: GetArticleRequest, headers?: object, signal?: AbortSignal): Promise<GetArticleResponse> => {
    return this.fetch(
      this.url('GetArticle'),
      createHttpRequest(JsonEncode('GetArticleRequest', req), headers, signal)).then((res) => {
      return buildResponse(res).then(_data => {
        return JsonDecode<GetArticleResponse>('GetArticleResponse', _data)
      })
    }, (error) => {
      throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
    })
