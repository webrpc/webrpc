...
func WebRPCSchemaHash() string {
	return "2c480c67fe13cd611d228efee1c10651af13443f"
}
...
type ExampleClient interface {
	...
	DownloadAvatarWithMeta(ctx context.Context, userId uint64) (*File, uint64, string, error)
}
...
func (c *exampleClient) DownloadAvatarWithMeta(ctx context.Context, userId uint64) (*File, uint64, string, error) {
	in := struct {
		Arg0 uint64 `json:"userId"`
	}{userId}

	out := struct {
		Ret1 uint64 `json:"size"`
		Ret2 string `json:"contentType"`
	}{}

	resp, err := doHTTPRequestRaw(ctx, c.client, c.urls[10], in, nil)
	if err != nil {
		if resp != nil {
			resp.Body.Close()
		}
		return nil, out.Ret1, out.Ret2, err
	}

	if err := decodeFileResponseMeta(resp, &out); err != nil {
		resp.Body.Close()
		return nil, out.Ret1, out.Ret2, err
	}

	return fileFromResponse(resp), out.Ret1, out.Ret2, nil
}
...
func (s *exampleService) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/v1/Example/DownloadAvatarWithMeta":
		handler = s.serveDownloadAvatarWithMetaJSON
	default:
...
func (s *exampleService) serveDownloadAvatarWithMetaJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	ctx = context.WithValue(ctx, MethodNameCtxKey, "DownloadAvatarWithMeta")
	...
	// Call service method implementation.
	ret0, ret1, ret2, err := s.ExampleServer.DownloadAvatarWithMeta(ctx, reqPayload.Arg0)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
	...
	if ret0 == nil {
		s.sendErrorJSON(w, r, ErrWebrpcBadResponse.WithCausef("method DownloadAvatarWithMeta returned a nil file"))
		return
	}
	metaPayload := struct {
		Ret1 uint64 `json:"size"`
	...
	metaBody, err := json.Marshal(metaPayload)
	if err != nil {
		s.sendErrorJSON(w, r, ErrWebrpcBadResponse.WithCausef("failed to marshal response metadata: %w", err))
		return
	}
	if err := setFileResponseMeta(w, metaBody); err != nil {
		s.sendErrorJSON(w, r, ErrWebrpcBadResponse.WithCause(err))
		return
	}
	serveFileResponse(w, ret0)
}
...
var methods = map[string]*method{
	...
	"/v1/Example/DownloadAvatarWithMeta": {
		name:        "DownloadAvatarWithMeta",
		service:     "Example",
		annotations: map[string]string{},
	},
}
...
// MaxFileResponseMetaSize caps the JSON-encoded metadata of a download
// method's non-file outputs. Servers, proxies and CDNs enforce their own
// limits on the header block (commonly 8-16 KiB in total), so oversized
// metadata is rejected at the source rather than silently truncated in
// transit. Metadata is for small values; anything larger belongs in a
// separate method.
const MaxFileResponseMetaSize = 4 << 10 // 4 KiB

// setFileResponseMeta sets the metadata header of a download response.
func setFileResponseMeta(w http.ResponseWriter, meta []byte) error {
	meta = asciiJSON(meta)
	if len(meta) > MaxFileResponseMetaSize {
		return fmt.Errorf("response metadata is %v bytes, over the %v byte limit of the %v header", len(meta), MaxFileResponseMetaSize, FileResponseMetaHeader)
	}
	w.Header().Set(FileResponseMetaHeader, string(meta))
	return nil
}

// asciiJSON rewrites every non-ASCII character of the JSON as a \uXXXX
// escape. HTTP header values are ASCII, and non-ASCII bytes in one are
// handled inconsistently across proxies and HTTP clients; the escapes are
// still valid JSON and decode back to the original text in any parser.
func asciiJSON(b []byte) []byte {
	...
	for _, r := range string(b) {
		switch {
		case r >= 0x20 && r < utf8.RuneSelf && r != 0x7f:
			out = append(out, byte(r))
		case r > 0xffff:
			// Outside the BMP: encode as a UTF-16 surrogate pair.
			r -= 0x10000
			out = appendUnicodeEscape(out, 0xd800+(r>>10))
			out = appendUnicodeEscape(out, 0xdc00+(r&0x3ff))
		default:
...
func isASCIIJSON(b []byte) bool {
	for _, c := range b {
		if c < 0x20 || c >= utf8.RuneSelf || c == 0x7f {
			return false
		}
	}
	return true
...
func appendUnicodeEscape(out []byte, r rune) []byte {
	...
	return append(out, '\\', 'u', hex[(r>>12)&0xf], hex[(r>>8)&0xf], hex[(r>>4)&0xf], hex[r&0xf])
}

// decodeFileResponseMeta decodes a download response's metadata header into
// out. A response without the header leaves out untouched, so a server that
// sends no metadata yields zero-valued outputs rather than an error.
func decodeFileResponseMeta(resp *http.Response, out interface{}) error {
	meta := resp.Header.Get(FileResponseMetaHeader)
	if meta == "" {
		return nil
	}
	if err := json.Unmarshal([]byte(meta), out); err != nil {
		return ErrWebrpcBadResponse.WithCausef("failed to unmarshal %v header: %w", FileResponseMetaHeader, err)
	}
	return nil
}
