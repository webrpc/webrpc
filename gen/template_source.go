...
// inferLocalTemplateVersion attempts to extract a git branch and commit slug from the TmplDir
// of a TemplateSource if its IsLocal flag is set and appends it to its TmplVersion.
// If the local directory is not under git control or some other problem happens,
// TmplVersion will be unchanged.
