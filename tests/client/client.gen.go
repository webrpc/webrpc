...
func (c *testApiClient) UploadFiles(ctx context.Context, files []*File) (uint32, error) {
	...
	parts := []filePart{}
	parts = append(parts, filePart{name: "files", files: files})
	err := doHTTPUploadRequest(ctx, c.client, c.urls[12], in, parts, &out)
...
func (c *testApiClient) DownloadFile(ctx context.Context, name string) (*File, error) {
	...
	resp, err := doHTTPRequestRaw(ctx, c.client, c.urls[13], in, nil)
	if err != nil {
		if resp != nil {
			resp.Body.Close()
		}
...
func (c *testApiClient) DownloadFileWithMeta(ctx context.Context, name string) (*File, uint64, string, error) {
	in := struct {
		Arg0 string `json:"name"`
	}{name}
	...
	resp, err := doHTTPRequestRaw(ctx, c.client, c.urls[14], in, nil)
	if err != nil {
		if resp != nil {
			resp.Body.Close()
		}
...
func (c *testApiClient) EchoFile(ctx context.Context, data *File) (*File, error) {
	...
	parts := []filePart{}
	parts = append(parts, filePart{name: "data", files: []*File{data}})
	resp, err := doHTTPUploadRequestRaw(ctx, c.client, c.urls[15], in, parts, nil)
	if err != nil {
		if resp != nil {
			resp.Body.Close()
		}
