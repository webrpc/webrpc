...
//
// webrpc-gen -schema=./schema/api.ridl -target=golang -pkg=client -client -out=./client/client.gen.go
package client
...
const ComplexApiPathPrefix = "/rpc/ComplexApi/"
const ErrorApiPathPrefix = "/rpc/ErrorApi/"

type complexApiClient struct {
	client HTTPClient
	urls   [2]string
}

func NewComplexApiClient(addr string, client HTTPClient) ComplexApi {
	prefix := urlBase(addr) + ComplexApiPathPrefix
	urls := [2]string{
		prefix + "GetComplex",
		prefix + "SendComplex",
	}
	return &complexApiClient{
		client: client,
		urls:   urls,
	}
}

func (c *complexApiClient) GetComplex(ctx context.Context) (*ComplexType, error) {
	out := struct {
...
func (c *complexApiClient) SendComplex(ctx context.Context, complex *ComplexType) error {
	in := struct {
